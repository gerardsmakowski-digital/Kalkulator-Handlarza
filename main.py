...
# --- ZAAWANSOWANY CSS (Stylizacja UI i Sidebar) ---
st.markdown("""
    <style>
    ...
    div[data-baseweb="segmented-control"] button {
        ...
        border: 1px solid #444 !important; padding: 10px 20px !important;
    }
...
with st.sidebar:
    ...
    # Opłaty dodatkowe (Układ poziomy)
    col_op1, col_op2 = st.columns(2)
    with col_op1:
        st.markdown("**Rejestracja**")
        rejestracja_check = st.checkbox("Zaznacz", value=True, label_visibility="collapsed")
    with col_op2:
        st.markdown("**Przegląd**") # Słowo w linii z rejestracją
    ...
    # Reszta pól w kolejności z arkusza
...
with t1:
    ...
    st.markdown(f"""<div class='table-container'>
        <div class='row'><span>Samochód (PLN)</span><span>{cena_pln_auto:,.2f} zł</span></div>
        ...
        <div class='row'><span>Lakierowanie ({ilosc_lakieru} elem.)</span><span>{koszt_lakiernika:,.2f} zł</span></div>
        <div class='row'><span>Mechanik i części</span><span>{mechanik + cena_czesci:,.2f} zł</span></div>
        <div class='row'><span>Opłaty (Przegląd/Rej.)</span><span>{koszt_prz + koszt_rej:,.2f} zł</span></div>
...
with t2:
    ...
    st.markdown(f"""<div class='table-container'>
        ...
        <div class='row'><span>Podatek (19%)</span><span>{dochod*0.19:,.2f} zł</span></div>
